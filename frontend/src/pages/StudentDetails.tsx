...
export default function StudentDetails() {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      await aiService.generateTasks(Number(id));
      await loadData(); // Recarrega para mostrar as novas tarefas
  ...
  const handleCreateMeeting = async (e: React.FormEvent) => {
    ...
    try {
      await meetingService.create({ date: newMeetingDate, topic: newMeetingTopic, guidance_id: Number(id) });
      alert("Solicitação enviada!");
      setNewMeetingDate(''); setNewMeetingTopic('');
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try { await taskService.updateStatus(taskId, newStatus); } 
    catch (error) { console.error("Erro ao mover", error); loadData(); }
  };
  ...
  const handleNotificationClick = async (notif: Notification) => {
    ...
    if (notif.link && notif.link !== window.location.pathname) { navigate(notif.link); }
    setShowNotif(false);
  };
  const unreadCount = notifications.filter(n => !n.read).length;

  const handleUpdateDefenseDate = async (newDate: string) => {
    try { await guidanceService.update(Number(id), { defense_date: newDate }); setGuidance({ ...guidance, defense_date: newDate }); } 
    catch (error) { alert("Erro ao atualizar data."); }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try { await attachmentService.upload(selectedTask.id, file); const updatedList = await attachmentService.getByTask(selectedTask.id); setAttachments(updatedList); alert("Sucesso!"); } 
    catch (error) { alert("Erro no upload."); } finally { setIsUploading(false); }
  };
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try { await taskService.create({ title: newTaskTitle, description: newTaskDesc, guidance_id: Number(id), time_estimate: newTaskDate || undefined }); setIsCreateModalOpen(false); setNewTaskTitle(''); setNewTaskDesc(''); setNewTaskDate(''); loadData(); } 
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-3">
              {/* BOTÃO MÁGICO DA IA (NOVO) */}
              <button 
                onClick={handleGenerateAI} 
                disabled={isGeneratingAI}
                className={`px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm text-white transition-all
                  ${isGeneratingAI ? 'bg-purple-400 cursor-wait' : 'bg-purple-600 hover:bg-purple-700 hover:shadow-md'}`}
              >
